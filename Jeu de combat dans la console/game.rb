class Personne
  ...
  def attaque(personne)
    ...
    if self.class == Joueur
      ...
      puts "#{@nom} a #{degats_bonus} pouvoirs pour attaquer."
    end
  ...
  def subit_attaque(degats_recus)
    ...
    puts "#{@nom} Vous êtes touché. Vous avez perdu #{degats_recus} % de vie !"
  end
end
...
class Joueur < Personne
  ...
  def soin
    ...
    if @points_de_vie >= 100
      puts "#{@nom} Vous ne pouvez pas vous soigner car vous êtes à 100 % de vie !!!"
    else
      s = 1 + Random.rand(40)
      ...
      puts "#{@nom} Vous venez de gagner #{s} % de vie (#{@points_de_vie}/100)."
    end
  end
  ...
  def ameliorer_degats
    ...
    # - Affiche ce qu'il s'est passé
    b = 5 + Random.rand(30)
    puts "#{@nom} a gagné #{b} % de pouvoirs de destruction !!!"
  end
end
...
class Monde
  ...
  def ennemis_en_vie
    # A faire:
    # - Ne retourner que les ennemis en 
    vivants = []
    @ennemis.each do |e|
      if e.en_vie
        vivants << e
